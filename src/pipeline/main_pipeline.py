...
class MainPipeline: 
    def __init__(self, audio_path, model_config=None, confi_thres = -0.25, resegment_minim = 2):
        """
        Construction for Pipeline object, once this is initiated the whole pipeline should be activated
        """
        # Set config params 
        ...
        # Here we run the transcription process run the whole pipeline
    ...
    def run_raw_transcription(self):
        """
        Run raw transcription on full audio
        """
        return self.model.transcribe_full(self.audio_path)
    
    def segment(self):
        """
        Segment seperately the silent and non-silent portions of the audio 
        """
    ...
    def silent_based_transcription(self):
        """
        Transcribe based on speech segments (segments we determined previously to not be silent)
        """
        new_transcriptions = []
    ...
    def confidence_based_segmentation(self):
        """
        We reranscribe/segment portions of the audio that has low confidence in terms of average low prob 
        """
        resemented_transcriptions = []
        
        # Loop through the non-silent transcriptions/segmentation
        for segment in self.speech_transcriptions:

            duration = segment["end"] - segment["start"]

            # Check if all the criterias are met 
            # (so we don't retranscribe audio portions that are either already too good or too short to be relevant)
...
#     """Test main (Ignore just for testing for debugging purposes)"""
